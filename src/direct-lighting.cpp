...
namespace local {
	...
	void gpu_cgls_lights_arealight_sampler::activate(rt_set *orig_set) {
		...
		set = *orig_set;
		set.rt = set.rt->copy();
		gpu_lights = gi::cuda::convert_and_upload_lights(nr_of_gpu_lights);
...
// 		gi::cuda::halton_pool2f pool = gi::cuda::generate_halton_pool_on_gpu(w*h);
		...
		set.bouncer = new gpu_cgls_arealight_evaluator<B, T>(w, h, gpu_materials, triangles, crgs, gpu_lights, nr_of_gpu_lights, pool, 32);
	...
	void gpu_cgls_lights_arealight_sampler::compute() {
		...
		gi::cuda::update_lights(gpu_lights, nr_of_gpu_lights);

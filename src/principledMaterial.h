...
namespace rta {
	...
	namespace Principled {
		...
		public:
			...
			PrincipledMaterial()
				:Material(), _mat(0) {
					_type = DIFFUSE; _diffuse = make_float3(0.f,0.f,0.f); _Tx = make_float3(0.f,0.f,0.f); _Ty = make_float3(0.f,0.f,0.f);
				}
			PrincipledMaterial(const rta::cuda::material_t *mat, const float2 &T, const float2 &upperT, const float2 &rightT, const float3 &Tx, const float3 &Ty)
				: Material(),_mat(mat) {
					_type = DIFFUSE;
